...
describe('Referral', () => {
  ...
  const USER = web3.Keypair.fromSecretKey(new Uint8Array([
  ...
  console.log('User', USER.publicKey.toBase58(), USER.secretKey)

  beforeAll(async () => {
    const signature = await provider.connection.requestAirdrop(USER.publicKey, LAMPORTS_AMOUNT_AIRDROP)
